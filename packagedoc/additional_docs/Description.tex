...
\subsection{GitHub Projects v2 Integration}

To retrieve \textbf{Sprint} and \textbf{Estimation} data from a GitHub Projects v2
board, add the following optional fields to the \pcode{github} tracker configuration:

\begin{itemize}
   \item \pcode{project\_number} (integer): The number shown in the project URL
         (e.g.\ \pcode{5} for \pcode{.../projects/5}).
   \item \pcode{project\_field\_mapping} (object): Maps internal issue attribute
         names to the exact field names configured in the GitHub Projects v2 board.
   \item \pcode{project\_field\_value\_mapping} (object): Optional per-field value
         translations. The key is the same internal attribute name; the value is a
         dict mapping raw GitHub field values (strings) to the target value used
         internally by the tool. Useful when a SingleSelect field uses custom string
         values (e.g.\ \plog{P0}, \plog{P1}) that must be converted to numbers.
\end{itemize}

Supported mapping keys and their corresponding GitHub Projects v2 field types:

\begin{center}
\begin{tabular}{|l|l|l|}
   \hline
   \textbf{Key} & \textbf{GitHub field type} & \textbf{Example field name} \\
   \hline
   \pcode{sprint}      & Iteration           & \plog{Sprint}    \\
   \hline
   \pcode{story\_point} & Number              & \plog{Estimate}  \\
   \hline
   \pcode{priority}    & Number / SingleSelect & \plog{Priority} \\
   \hline
   \pcode{status}      & SingleSelect        & \plog{Status}    \\
   \hline
\end{tabular}
\end{center}

When \pcode{project\_number} is omitted the tool falls back to label-based
...
\subsection{Planning Master Configuration}

The \pcode{is_master} flag (boolean, default \pcode{false}) declares which
tracker is authoritative for planning information (sprint, story point, priority,
assignee). It can be added to any tracker section in the configuration.

The effective planning master is resolved as follows:

\begin{center}
\begin{tabular}{|l|l|l|}
   \hline
   \textbf{org.is\_master} & \textbf{des.is\_master} & \textbf{Planning master} \\
   \hline
   not set or false & not set or false & Destination \textit{(backward-compatible default)} \\
   \hline
   \pcode{true}    & not set or false & Source (original) \\
   \hline
   not set or false & \pcode{true}    & Destination \\
   \hline
   \pcode{true}    & \pcode{true}    & Destination \\
   \hline
\end{tabular}
\end{center}

\textbf{Destination is master} (default): Sprint, story point, priority and
assignee are read from the destination issue and synced back to the original
tracker.

\textbf{Source is master}: Planning information from the original issue is
pushed to the destination tracker and overrides the destination values.
No sync-back to the original is performed.

Example — GitHub as planning master syncing to RTC:
\begin{pythoncode}
{
   "source": ["github"],
   "destination": ["rtc"],
...
\begin{longtable}{|l|l|l|l|}
   ...
   \multirow{12}{*}{\textbf{Sync}}
      ...
      \cline{2-4}
      & \multirow{2}{*}{Sprint}
         & $\leftarrow$ & Sprint (if destination is master) \\
         \cline{3-4}
         & & $\rightarrow$ & Sprint (if source is master) \\
      \cline{2-4}
      & Version & $\leftarrow$ & Version (if destination is master) \\
      \cline{2-4}
      & \multirow{2}{*}{Assignee}
         & $\leftarrow$ & Assignee (if set on Destination, destination is master) \\
         \cline{3-4}
         & & $\rightarrow$ & Assignee (if not set on Destination, or source is master) \\
      \cline{2-4}
      & \multirow{2}{*}{Story point}
         & $\leftarrow$ & Story point (if set on Destination, destination is master) \\
         \cline{3-4}
         & & $\rightarrow$ & Story point (if not set on Destination, or source is master) \\
      \cline{2-4}
      & \multirow{2}{*}{Priority}
         & $\leftarrow$ & Priority (if set on Destination, destination is master) \\
         \cline{3-4}
         & & $\rightarrow$ & Priority (if not set on Destination, or source is master) \\
   \hline
\end{longtable}
...
\begin{longtable}{|l|l|l|l|l|}
   ...
   \hline
   Sprint & Labels (PI.*) or Projects v2 & Labels (PI.*) & Sprint & Planned For \\
   \hline
   Estimation & Labels (x pts) or Projects v2 & Labels (x pts) & Estimate & Story point \\
   \hline
\end{longtable}